...
function updateEncyclopedia() {
  ...
  const enemyEntries = [
    ...
    },
    {
      key: "saboteur",
      name: "Saboteur",
      desc: "Disables traps and stuns towers briefly.",
    },
    {
      key: "chimera",
      name: "Chimera",
...
function createEnemy(type, options = {}) {
  ...
  const armoredFlag = options.armored || isDiamond || isChimera || false;
...
function updateSpikeTower(tower, dt, stats) {
  ...
  if (phase === "extend") {
    ...
    const hits = applyLaneDamage(len, spikeDamage, spikeSlow, spikeHold);
    if (hits > 0 || next >= 1) {
...
function updateEnemies(dt) {
  ...
  for (const enemy of state.enemies) {
    ...
    if (enemy.troll) {
      ...
      if (enemy.backtrackCooldown <= 0 && Math.random() < 0.08) {
