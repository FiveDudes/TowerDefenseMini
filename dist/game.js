...
window.APPWRITE_ENDPOINT_KEY = APPWRITE_ENDPOINT_KEY;
window.APPWRITE_PROJECT_KEY = APPWRITE_PROJECT_KEY;
...
function handleWaveAlerts(wave) {
  ...
  const unlocks = waveUnlocks.filter((entry) => entry.wave !== 1 && entry.wave === wave);
...
function drawProjectiles() {
  for (const proj of state.projectiles) {
    ...
    if (proj.kind === "bomb") {
      ...
      ctx.ellipse(0, 0, 8, 10, 0, 0, Math.PI * 2);
      ctx.stroke();
      ctx.fillStyle = "#2f4e25";
      ctx.beginPath();
      ctx.roundRect(-3.5, -13, 7, 6, 2);
      ctx.fill();
      ctx.strokeStyle = "#dfe7d3";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.arc(5, -14, 3.2, 0, Math.PI * 2);
      ctx.stroke();
      ctx.strokeStyle = "#8aa0ba";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(5, -14);
      ctx.lineTo(11, -19);
...
function drawNukeLaunches() {
  for (const launch of state.nukeLaunches) {
    ...
    ctx.fillStyle = "#8a8f5a";
    ctx.beginPath();
    ctx.ellipse(0, 2, 12, 19, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "#2d2c24";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.ellipse(0, 2, 12, 19, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.fillStyle = "#e0e53f";
